...
void DisplayManager::init() {
    ...
    const size_t draw_rows = 40;
    buffer_size = screen_width * draw_rows * sizeof(uint16_t);

    draw_buffer = static_cast<lv_color_t*>(heap_caps_aligned_alloc(
    ...
    while (dma_staging_rows >= 4 && dma_staging_buffer == nullptr) {
        dma_staging_buffer = static_cast<uint16_t*>(heap_caps_aligned_alloc(
            LV_DRAW_BUF_ALIGN, screen_width * dma_staging_rows * sizeof(uint16_t),
            MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA | MALLOC_CAP_8BIT));
        if (!dma_staging_buffer) dma_staging_rows /= 2;
    }
    if (!dma_staging_buffer) return;
...
void DisplayManager::touchpad_read_cb(lv_indev_t*, lv_indev_data_t* data) {
...
uint32_t DisplayManager::millis_cb() { return millis(); }
...
void DisplayManager::set_brightness(float brightness) {
    ...
    display->setBrightness((uint8_t)(brightness * 255.0f));
}
